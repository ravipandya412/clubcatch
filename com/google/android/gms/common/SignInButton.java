...
public final class SignInButton extends FrameLayout implements OnClickListener {
    ...
    private static Button m136c(Context context, int i, int i2) {
        Button etVar = new et(context);
        etVar.m732a(context.getResources(), i, i2);
        return etVar;
    }

    private void m137v(Context context) {
        if (this.yY != null) {
            removeView(this.yY);
        }
        try {
            this.yY = es.m1812d(context, this.mSize, this.yX);
        } catch (C0253a e) {
            Log.w("SignInButton", "Sign in button not found, using placeholder instead");
            this.yY = m136c(context, this.mSize, this.yX);
        }
        addView(this.yY);
    ...
    public void setStyle(int buttonSize, int colorScheme) {
        ...
        er.m721a(z2, "Unknown button size " + buttonSize);
        if (colorScheme < 0 || colorScheme >= SIZE_ICON_ONLY) {
            z = false;
        }
        er.m721a(z, "Unknown color scheme " + colorScheme);
