...
public class ToolbarWidgetWrapper implements DecorToolbar {
    ...
    class C08812 extends ViewPropertyAnimatorListenerAdapter {
    ...
    class C08823 extends ViewPropertyAnimatorListenerAdapter {
        C08823() {
        }
    ...
    public ToolbarWidgetWrapper(Toolbar toolbar, boolean style) {
        this(toolbar, style, C0103R.string.abc_action_bar_up_description, C0103R.drawable.abc_ic_ab_back_mtrl_am_alpha);
    }
    ...
    public ToolbarWidgetWrapper(Toolbar toolbar, boolean style, int defaultNavigationContentDescription, int defaultNavigationIcon) {
        ...
        if (style) {
            TintTypedArray a = TintTypedArray.obtainStyledAttributes(toolbar.getContext(), null, C0103R.styleable.ActionBar, C0103R.attr.actionBarStyle, 0);
            CharSequence title = a.getText(C0103R.styleable.ActionBar_title);
            if (!TextUtils.isEmpty(title)) {
                setTitle(title);
            }
            CharSequence subtitle = a.getText(C0103R.styleable.ActionBar_subtitle);
            if (!TextUtils.isEmpty(subtitle)) {
                setSubtitle(subtitle);
            }
            Drawable logo = a.getDrawable(C0103R.styleable.ActionBar_logo);
            if (logo != null) {
                setLogo(logo);
            }
            Drawable icon = a.getDrawable(C0103R.styleable.ActionBar_icon);
            if (this.mNavIcon == null && icon != null) {
                setIcon(icon);
            }
            Drawable navIcon = a.getDrawable(C0103R.styleable.ActionBar_homeAsUpIndicator);
            if (navIcon != null) {
                setNavigationIcon(navIcon);
            }
            setDisplayOptions(a.getInt(C0103R.styleable.ActionBar_displayOptions, 0));
            int customNavId = a.getResourceId(C0103R.styleable.ActionBar_customNavigationLayout, 0);
            if (customNavId != 0) {
                setCustomView(LayoutInflater.from(this.mToolbar.getContext()).inflate(customNavId, this.mToolbar, false));
                setDisplayOptions(this.mDisplayOpts | 16);
            }
            int height = a.getLayoutDimension(C0103R.styleable.ActionBar_height, 0);
            if (height > 0) {
                LayoutParams lp = this.mToolbar.getLayoutParams();
                lp.height = height;
                this.mToolbar.setLayoutParams(lp);
            }
            int contentInsetStart = a.getDimensionPixelOffset(C0103R.styleable.ActionBar_contentInsetStart, -1);
            int contentInsetEnd = a.getDimensionPixelOffset(C0103R.styleable.ActionBar_contentInsetEnd, -1);
            if (contentInsetStart >= 0 || contentInsetEnd >= 0) {
                this.mToolbar.setContentInsetsRelative(Math.max(contentInsetStart, 0), Math.max(contentInsetEnd, 0));
            }
            int titleTextStyle = a.getResourceId(C0103R.styleable.ActionBar_titleTextStyle, 0);
            if (titleTextStyle != 0) {
                this.mToolbar.setTitleTextAppearance(this.mToolbar.getContext(), titleTextStyle);
            }
            int subtitleTextStyle = a.getResourceId(C0103R.styleable.ActionBar_subtitleTextStyle, 0);
            if (subtitleTextStyle != 0) {
                this.mToolbar.setSubtitleTextAppearance(this.mToolbar.getContext(), subtitleTextStyle);
            }
            int popupTheme = a.getResourceId(C0103R.styleable.ActionBar_popupTheme, 0);
            if (popupTheme != 0) {
        ...
        this.mToolbar.setNavigationOnClickListener(new C01221());
    }
    ...
    public void setMenu(Menu menu, MenuPresenter.Callback cb) {
        if (this.mActionMenuPresenter == null) {
            ...
            this.mActionMenuPresenter.setId(C0103R.id.action_menu_presenter);
        }
    ...
    private void ensureSpinner() {
        if (this.mSpinner == null) {
            this.mSpinner = new SpinnerCompat(getContext(), null, C0103R.attr.actionDropDownStyle);
    ...
    public void animateToVisibility(int visibility) {
        if (visibility == 8) {
            ViewCompat.animate(this.mToolbar).alpha(0.0f).setListener(new C08812());
        } else if (visibility == 0) {
            ViewCompat.animate(this.mToolbar).alpha(1.0f).setListener(new C08823());
        }
    }
